...
class MLP():
    ...
    def __init__(self, inputLsize, hiddenLsize, outputLsize, learning_rate):
        ...
        self.w1 = np.random.randn(inputLsize, hiddenLsize) * np.sqrt(1/hiddenLsize)
        self.b1 = np.random.randn(1, hiddenLsize) * np.sqrt(1/hiddenLsize)
        self.w2 = np.random.randn(hiddenLsize, outputLsize) * np.sqrt(1/outputLsize)
        self.b2 = np.random.randn(1, outputLsize) * np.sqrt(1/outputLsize)
    ...
    def forward(self, x):
        ...
        Set z1, a1, z2, a2
        '''
        self.z1 = (x @ self.w1 + self.b1).numpy()
    ...
    def backward(self, x, y_onehot):
        ...
        '''
        self.d_a2 = (self.a2 - y_onehot) * 2
        self.d_z2 = sigmoid_deriv(self.z2) * self.d_a2
        self.d_b2 = self.d_z2
        self.d_w2 = self.a1.T * self.d_z2
        self.d_a1 = self.d_z2 @ self.w2.T
        self.d_z1 = sigmoid_deriv(self.z1) * self.d_a1
        self.d_b1 = self.d_z1
        self.d_w1 = (x.T).numpy() * self.d_z1
